...
이 이미지가 제공하는 기본 환경설정은 Neo4j을 학습하기 위해서 제작되었지만, 생산용으로 적합하도록 수정해야 합니다. 특히, Neo4j에 할당된 메모리는 다수의 컨테이너가 같은 서버에서 시작되기 위해서 제한적 입니다. (아래 ```NEO4J_CACHE_MEMORY```와 ```NEO4J_HEAP_MEMORY```를 참조하시오.) 자세한 내용은 [섹션 A.1, 설정 세팅]("https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/")를 참조하시오. 
...
이것은 이미지 커스터마이징 양을 기준으로 선택합니다. 



#### 2.5.3.1 환경 변수 

작동할 때, 환경 변수를 컨테이너에 전송하십시오.
...
```dbms_jvm_additional```와 같이 다양한 옵션을 택하는 변수는 한 번만 정의해야하고, 여러 값을 연결해야 합니다. 
예를 들면:
...
남은 섹션에서는 Neo4j 앤터프라이즈 버전에서쓸 수 있는 통제 제어 설정에 대해 다룹니다.  

+ ```NEO4J_dbms_mode```: 데이터베이스는 기본 ```SINGLE```으로 설정되고, 일반적인 클러스터링으로 ```CORE```이나 ```READ_REPLICA```로 설정되고, 하이 클러스터링에는 ```HA```로 설정됩니다.

####일반적인 클러스터 설정

+ ```NEO4J_causal__clustering_expected__core__cluster__size``` : 처음에 초기 클러스터 사이즈(코어 인스턴스 개수).
...
클러스팅 설정을 다루기 위한 주소와 포트는 아래를 참조하면 됩니다. 


####고 가용성 클러스터 설정

+ ```NEO4J_ha_server__id``` : 서버의 아이디는 클러스터 내에서 고유한 값이어야 딥니다. 

+ ```NEO4J_ha_host_coordination``` : HA 모드에서 클러스터 조정에 쓰이는 주소(포트를 포함하여)는, 모든 클러스터 멤버가 접근할 수 있어야 합니다. 

+ ```NEO4J_ha_host_data``` : HA 모드에서 데이터 전송에 쓰이는 주소는(포트 포함), 모든 클러스터 멤버가 접근할 수 있어야 합니다. 

+ ```NEO4J_ha_initial__hosts``` : 쉼표로 구분 된 클러스터의 다른  구성원 목록 
 


HA 클러스터 환경 설정의 예는 아래에서 볼 수 있습니다. 
...
/conf 볼륨의 설정 파일이 이미지가 제공한 파일을 오버라이드할 것 입니다. 그렇기 때문에, 파일에서 하나의 값을 바꾸고 싶다면 나머지 파일이 바르고 완전한지 확인해야 합니다. 도커가 컨테이너에 전송한 환경 변수는 /conf 볼륨 내에 설정 파일 값을 오버라이드할 것 입니다. 
...
개별 설정을 변경할 경우, 이를 스크렙트에서 적용할 수 있도록 후크를 제공합니다. 

```
COPY extra_conf.sh /extra_conf.sh
```

실행될 때 ```EXTENSION_SCRIPT``` 환경 변수전달해 스크립트를 소스로 사용할 수 있습니다. 

```
docker run -e "EXTENSION_SCRIPT=/extra_conf.sh" cafe12345678
```

확장 스크립트가 소스가되면 현재 작업 디렉토리는 Neo4j 설치 루트가됩니다.

####2.5.3.4. Neo4j 업그레이드

업그레이드를 하려면, ```NEO4J_dbms_allow__upgrade```을 ```참```으로 설정하면 됩니다. 업그레이드 관련 자세한 내용은 아래를 참고하면 됩니다.:

+ [싱글 인스턴스 업그레이드]("https://neo4j.com/docs/operations-manual/current/upgrade/deployment-upgrading/")
+ [Neo4j 일반 클러스터 업그레이드]("https://neo4j.com/docs/operations-manual/current/upgrade/causal-cluster/")
+ [Neo4j HA 클러스터 업그레이드]("https://neo4j.com/docs/operations-manual/current/upgrade/high-availability/")


###2.5.4. Neo4j 일반 클러스터 모드

Docker에서 CC모드로 Neo4j를 작동하려면 서로 통신하도록 클러스터 컨테이너를 연결해야 됩니다. 각 컨테이너는 서로에 대한 네트워크 루트 및 ```NEO4J_causal__clustering_expected__core__cluster__size```와 ```NEO4J_causal__clustering_initial__discovery__members```를 가져야되고 환경 변수 코어 설정을 해야합니다. 읽기 복제본은 ```NEO4J_causal__clustering_initial__discovery__members```을 유지해야 합니다. 

단일 Docker 호스트 내에서 아래와 같이 실행됩니다. HTTP, HTTPS와 Bolt가 기본 포트 입니다. 각 컨테이너에서 이 포트들은 
Docker 호스트의 다른 포트에 매핑됩니다. 
...
추가 인스턴스는 임시 모드로 클러스터에 추가될 수 있습니다. 읽기 복제본 예는 아래와 같이 추가 가능합니다.:
...
각 컨테이너가 실제 컴퓨터에서 실행 중이고 Docker 네트워크가 사용되지 않을 경우에 실제 컴퓨터기리 통신할 수 있도록 주소를 정의해야 됩니다. 또한, 각 컨테이너를 호스트 기계 네트워크에 연결해야 합니다. 
...
이는 ```<public-address>``` 가 공공 호스트 이름이거나 기계의 ip-주소입니다. 더 많은 정보는[섹션 4.2.3, 새로운 일상 클러스터 제작]("https://neo4j.com/docs/operations-manual/current/clustering/causal-clustering/setup-new-cluster/")에서 확인할 수 있습니다. 
