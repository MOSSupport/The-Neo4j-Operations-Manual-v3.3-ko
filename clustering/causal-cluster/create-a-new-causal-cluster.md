...
#### 4.2.3.6. 클러스터 멤버의 초기 발견
Neo4j 코어 서버가 시작되면 다른 코어 멤버들과 연락하고 새로운 클러스터를 형성하거나 기존 클러스터에 가입해야 합니다. 다른 멤버에게 연락하려면 코어 서버가 그들의 주소를 알아야 합니다. 주소는 호스트 이름 또는 IP 주소와 검색 서비스가 수신 대기 중인 포트로 구성됩니다.

클러스터 멤버가 다른 멤버를 발견하면, 나머지 클러스터에 대해 알고있는 내용을 공유합니다. 따라서 각 인스턴스가 클러스터의 다른 모든 인스턴스의 주소를 가질 필요는 없습니다. 그러나 초기 발견 멤버가 둘 이상의 분리된 세트를 형성한다면 클러스터를 형성할 수 없습니다.

다른 클러스터 멤버의 주소를 알고있는 경우, 명시적으로 나열할 수 있습니다. 이 경우 우리는 기본 `causal_clustering.discovery_type=LIST`을 사용할 수 있고, 각 시스템의 구성에서 주소를 하드 코딩할 수 있습니다.(예: `causal_clustering.initial_discovery_members=10.0.0.1:5000,10.0.0.2:5000,10.0.0.3:5000`)

발견을 위해 코어 멤버의 주소를 명시적으로 나열하는 것은 편리하지만 제한 사항이 있습니다.
* 첫째, 코어 멤버가 교체되고 새 멤버의 주소가 다를 경우, 목록이 뒤떨어지게 됩니다. 구 멤버와 동일한 주소를 통해 새 멤버에 도달할 수 있도록 보장함으로써 구식 목록을 피할 수는 있지만 항상 실용적인 것은 아닙니다.
*둘째, 일부 상황에서는 클러스터를 구성할 때 주소를 알 수 없습니다. 예를 들어 Causal 클러스터를 배포하기 위해 컨테이너 편성을 사용할 때의 경우가 될 수 있습니다.

실용적이지 않거나 발견할 클러스터 멤버의 주소를 명시적으로 나열할 수 없는 경우, DNS를 사용하여 추가 발견 메커니즘이 제공됩니다.

#### 4.2.3.7. DNS를 사용한 클러스터 멤버의 초기 발견
기본 `LIST` 발견 유형 외에도 발견을 위해 코어 클러스터 멤버의 주소를 얻는 데 사용될 수 있는 두 가지 메커니즘이 있습니다. 두 방법은 모두 DNS를 사용합니다.
* `config_causal_clustering.discovery_type=DNS`를 사용하면 연락할 IP 주소를 발견하기 위해 초기 발견 멤버가 DNS A 레코드에서 확인될 것입니다.
* `config_causal_clustering.discovery_type=SRV`를 사용하면 IP 주소/호스트 이름 및 접속할 발견 서비스 포트를 발견하기 위해 초기 발견 멤버가 DNS SRV 레코드에서 확인될 것입니다.
